...
export function dgct(fl) {
    ...
    if (fl) {
        setTimeout(() => {
            ...
            let inputKiemKhuVuc = document.getElementById('kiemKhuVuc')
            // Lọc khu vực (ẩn hiện)
            let divSpanLoc = document.getElementsByClassName('spanLoc');
            let spanLoc = divSpanLoc[0].children;
            let divLocKhuVuc = document.getElementById('locKhuVuc');
            let pLocKhuVuc = divLocKhuVuc.children;
            // Lọc theo mức lương
            let spanLocTien = document.getElementById('Tien');
            let inputKiemTien = document.getElementById('kiemTien');
            let divLocTien = document.getElementById('locTien');
            spanLocTien.addEventListener('click', function () {
                if (divLocTien.style.display === 'none') {
                    divLocTien.style.display = 'block';
                }
                else {
                    divLocTien.style.display = 'none';
                }
            })
            let childrenDivLocTien = divLocTien.children;
            for (let i = 0; i < childrenDivLocTien.length; i++) {
                childrenDivLocTien[i].addEventListener('click', function () {
                    inputKiemTien.value = childrenDivLocTien[i].textContent;
                });
            }

            spanLoc[0].addEventListener('click', function () {
                if (divLocKhuVuc.style.display === 'none') {
                    divLocKhuVuc.style.display = 'block';
                }
                else {
                    divLocKhuVuc.style.display = 'none';
                }
            })
            for (let i = 0; i < pLocKhuVuc.length; i++) {
                pLocKhuVuc[i].addEventListener('click', function () {
                    inputKiemKhuVuc.value = pLocKhuVuc[i].textContent;
                })
            }
...
export function tuyenDung() {
    ...
    let pNoiDung = document.getElementById('noiDungBaiViet');
    let pMail = document.getElementById('email');
    const pArray = [pTenCongTy, pViTri, pNoiDung, pMucLuong, pSoDienThoai, pMail];
